...
public interface CourseRepository extends JpaRepository<Course, Long>, CourseRepositoryCustom {
    ...
    @Query("SELECT DISTINCT c FROM Course c " +
            "LEFT JOIN c.instructorEnrollments ie " + // Liên kết với entity InstructorEnrollment
            "LEFT JOIN ie.instructor u " + // Liên kết với entity Instructor thông qua InstructorEnrollment
            "LEFT JOIN c.lessons l " + // Liên kết với entity Lesson
            ...
            "AND (:createdDate IS NULL OR c.startDate = :createdDate) " +
    ...
    @Query("SELECT new com.example.backend.dto.response.InstructorResp(u.id, u.name, u.userCode, u.email, u.phone, u.dateOfBirth, u.roleId, u.statusCode, u.experience) "
            + "FROM InstructorEnrollment ie JOIN ie.instructor u WHERE ie.course.id = :courseId")
    ...
    @Query("SELECT new com.example.backend.dto.response.CourseResp(c.id, c.courseCode, c.courseName, c.description, "
            + "c.learningOutcome, c.lessonCount, c.startDate, c.endDate, c.statusCode, c.backgroundImg) "
            + "FROM Course c "
            + "JOIN InstructorEnrollment ie ON c.id = ie.course.id "
            + "JOIN User u ON ie.instructor.id = u.id "
            + "WHERE u.userCode = :currentID")
    List<CourseResp> findCoursesByInstructor(@Param("currentID") String currentID);
