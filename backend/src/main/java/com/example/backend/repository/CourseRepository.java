...
public interface CourseRepository extends JpaRepository<Course, Long>, CourseRepositoryCustom {
  ...
  @Query("SELECT new com.example.backend.dto.response.InstructorResp(i.instructor.id, i.instructor.name, i.instructor.userCode) "
      +
      "FROM Instructor i WHERE i.course.id = :courseId")
  ...
  /**
   * Tìm thông tin khóa học dưới dạng {@link CourseResp} bằng {@code courseId}.
   *
   * @param courseId ID khóa học
   * @return Optional chứa thông tin khóa học dưới dạng {@link CourseResp}
   */
  @Query("SELECT new com.example.backend.dto.response.LessonSimpleResp(l.id, l.lessonName, l.videoLink, l.resourceLink) "
      +
      "FROM LessonDetail l WHERE l.course.id = :courseId ORDER BY l.lessonOrder ASC")
  List<LessonSimpleResp> findSimpleLessonsByCourseId(@Param("courseId") Long courseId);

  /**
   * Tìm danh sách bài học đơn giản của khóa học dưới dạng
   * {@link LessonSimpleResp}.
   *
   * @param courseId ID khóa học
   * @return Danh sách các bài học đơn giản của khóa học dưới dạng
   *         {@link LessonSimpleResp}
   */
  @Query("SELECT new com.example.backend.dto.response.LessonSimpleResp(l.id, l.lessonName, l.videoLink, l.resourceLink) "
      +
